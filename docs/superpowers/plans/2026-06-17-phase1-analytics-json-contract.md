...
>
> Note 2 — degenerate passers (IMPLEMENTED in Task 11): a net-cash name (e.g. JUSTDIAL) has `enterprise_value <= 0.05 * EBITDA`, so sponsor equity is near-zero and `run_lbo` returns a *finite but absurd* MOIC (8680x) — which `_json_safe` does NOT catch (it only nulls NaN/inf). `build_company_block` therefore checks `_is_degenerate(res, entry_ebitda)` (same 0.05*EBITDA cutoff as `tools/sitegen/returns.py` / `export_site.py`) and, when true, returns a flagged stub: all `COMPANY_KEYS` present, `returns.irr/moic = None`, `returns.degenerate = True`, and the heavy analytics (`montecarlo`/`downside`/`sensitivity`/`solvers`/`sobol`) set to `None` (skipped — garbage and slow). `feasibility` and `delisting` are still computed (promoter/float signals, independent of LBO math). The `passers[]` summary carries a `degenerate` boolean. `test_degenerate_company_block_is_flagged_not_absurd` locks this in.
